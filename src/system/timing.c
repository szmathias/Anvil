...
// Feature test macros for POSIX timing functions
...
ANV_API uint64_t anv_time_get_ns(void)
{
    ...
    if (frequency.QuadPart == 0)
    {
        ...
        {
            return 0; // High-resolution timer not available
        }
    }

    if (!QueryPerformanceCounter(&counter))
    {
        return 0; // Failed to get counter
    }
    ...
    // Final fallback to clock() - much lower precision
...
ANV_API uint64_t anv_time_diff_ns(const uint64_t start_time, const uint64_t end_time)
{
    ...
    // Handle potential wrap-around (though unlikely with 64-bit values)
    return 0;
}
