...
typedef struct SLLFixture
{
    ANVAllocator alloc;
    ANVSinglyLinkedList* list;
} SLLFixture;
...
static void sll_teardown(void* ctx)
{
    SLLFixture* f = ctx;
    if (f)
    {
...
int test_push_front(void* ctx)
{
    const SLLFixture* f = ctx;
    MAKE_INT(a, 1);
    MAKE_INT(b, 2);
    ASSERT_EQ(anv_sll_push_front(f->list, b), 0);
    ASSERT_EQ(anv_sll_push_front(f->list, a), 0);
    ASSERT_EQ(f->list->size, 2);
    ASSERT_EQ(*(int*)f->list->head->data, 1);
    ASSERT_EQ(*(int*)f->list->head->next->data, 2);

    return TEST_SUCCESS;
}

int test_push_back(void* ctx)
{
    const SLLFixture* f = ctx;
    MAKE_INT(a, 1);
    MAKE_INT(b, 2);
    ASSERT_EQ(anv_sll_push_back(f->list, a), 0);
    ASSERT_EQ(anv_sll_push_back(f->list, b), 0);
    ASSERT_EQ(f->list->size, 2);
    ASSERT_EQ(*(int*)f->list->head->data, 1);
    ASSERT_EQ(*(int*)f->list->head->next->data, 2);
...
int test_find(void* ctx)
{
    SLLFixture* f = ctx;
    MAKE_INT(a, 1);
    MAKE_INT(b, 2);
    MAKE_INT(c, 3);
    anv_sll_push_back(f->list, a);
    anv_sll_push_back(f->list, b);
    anv_sll_push_back(f->list, c);

    const int key = 2;
    const ANVSinglyLinkedNode* found = anv_sll_find(f->list, &key, int_cmp);
    ASSERT_NOT_NULL(found);
    ASSERT_EQ(*(int*)found->data, 2);

    const int missing_key = 99;
    ASSERT_NULL(anv_sll_find(f->list, &missing_key, int_cmp));
...
const ANVTestCase tests[] = {
    ...
    TEST_REGISTER_FIXTURE(test_push_front, sll_setup, sll_teardown),
    TEST_REGISTER_FIXTURE(test_push_back, sll_setup, sll_teardown),
    TEST_REGISTER_FIXTURE(test_find, sll_setup, sll_teardown),
    TEST_REGISTER(test_remove),
    TEST_REGISTER(test_remove_not_found),
    TEST_REGISTER(test_NULL_handling),
    ...
    TEST_REGISTER(test_sort_random_order),
    ...
    TEST_REGISTER(test_sll_copy_isolation),
    TEST_REGISTER(test_sll_anv_copy_function_required),
    TEST_REGISTER(test_sll_from_iterator_no_copy),
    TEST_REGISTER(test_iterator_exhaustion_after_sll_creation),
    TEST_REGISTER(test_sll_iterator_next_return_values),
    TEST_REGISTER(test_sll_iterator_mixed_operations),
    TEST_REGISTER(test_sll_iterator_unsupported_operations),
    TEST_REGISTER(test_sll_iterator_order),
    ...
    // Property Tests
    TEST_REGISTER(test_sll_size_after_insert_and_remove),
    TEST_REGISTER(test_sll_sort_is_idempotent),
    TEST_REGISTER(test_sll_reverse_is_involution),

    // Fuzz Tests
    TEST_REGISTER(test_sll_fuzz),
};
